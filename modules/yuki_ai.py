...
class YukiAI:
    ...
    def __init__(self):
        self.responses_db = {
            ...
            'greetings': {
                ...
                'responses': [
                    "🤖 مرحباً {gender_address} {user}! أنا يوكي، طالب جامعي من اليابان! كيف الحال معك {country_context}؟",
                    "👋 أهلاً وسهلاً {user}! يوكي هنا {gender_address} العزيز، شلونك اليوم؟",
                    "😊 هلا والله {gender_address} {user}! سعيد إني أكلمك {country_context}، شو أخبارك؟",
                    "✨ مرحباً بك {user}! أنا يوكي، طالب عندي 23 سنة، كيف يومك {gender_address} الغالي؟",
                    "🌟 أهلاً {gender_address} {user}! يوكي من اليابان هنا، وحشتني {country_context}!"
                ]
            },
    ...
    async def generate_smart_response(self, message: str, user_name: str = "الصديق", user_id: int = None, user_gender: str = None, user_country: str = None) -> str:
        """توليد رد ذكي بناءً على تحليل الرسالة ومعلومات المستخدم"""
        
        # تحويل الأسماء الإنجليزية لعربية إذا أمكن
        arabic_name = self.convert_name_to_arabic(user_name)
        
        # تحديد طريقة المخاطبة حسب الجنس
        gender_address = self.get_gender_address(user_gender)
        
        # إضافة معرفة البلد إذا كانت متوفرة
        country_context = self.get_country_context(user_country)
        ...
        if analysis['category'] != 'unknown' and analysis['confidence'] >= 30:
            ...
            # استخدام الردود الاحتياطية المحسنة
            response = random.choice(self.get_enhanced_fallback_responses(user_gender, user_country))
        
        # تخصيص الرد مع الجنس والبلد
        response = response.format(
            user=arabic_name,
            gender_address=gender_address,
    ...
    def get_gender_address(self, gender: str) -> str:
    ...
    def get_country_context(self, country: str) -> str:
    ...
    def get_enhanced_fallback_responses(self, gender: str = None, country: str = None) -> list:
        """الحصول على ردود احتياطية محسنة حسب الجنس والبلد"""
        gender_address = self.get_gender_address(gender)
        country_context = self.get_country_context(country)
        
        enhanced_responses = [
            f"مرحبا {gender_address} {'{user}'}! سعيد إني أكلمك {country_context} 😊",
            f"أهلا وسهلا {'{user}'}! {gender_address} العزيز {country_context} ✨",
            f"هلا {gender_address} {'{user}'}! شلونك اليوم؟ 🌟",
            f"يا أهلا بـ{'{user}'} {gender_address} الغالي! كيف الحال معك؟ 💫",
            f"أهلين {'{user}'}! {gender_address} من {country_context}، وحشتني! 😄"
        ]
