    ...
    async def add_xp(self, user_id, action_type="message"):
        try:
            ...
            if not level_data:
                ...
                try:
                    # التعامل مع نتائج قاعدة البيانات بطريقة آمنة
                    if level_data:
                        # معالجة البيانات حسب النوع
                        if hasattr(level_data, 'get'):  # Dict-like object
                            current_xp = max(0, level_data.get('xp', 0))
                            current_world = level_data.get('world_name', "عالم النجوم")
                            current_level = level_data.get('level_name', "نجم 1")
                        elif hasattr(level_data, '__len__') and len(level_data) > 3:  # List/tuple
                            current_xp = max(0, level_data[1] if level_data[1] is not None else 0)
                            current_world = level_data[3] if level_data[3] else "عالم النجوم"
                            current_level = level_data[2] if level_data[2] else "نجم 1"
                        else:
                            current_xp = 0
                            current_world = "عالم النجوم"
                            current_level = "نجم 1"
