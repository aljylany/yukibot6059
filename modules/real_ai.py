...
class RealYukiAI:
    ...
    def __init__(self):
        ...
        self.system_prompt = """أنت يوكي، صديق المجموعة الذكي والمرح! طورك يوكي براندون ليكون مساعد طبيعي وودود للجميع.

تحدث بطبيعية وبساطة. أنت صديق حقيقي، ليس مجرد بوت. عندما يسألك أحد عن نفسه أو اللعبة ستحصل على بياناته الحقيقية - لكن لا تذكر هذا إلا إذا احتجت لاستخدامها فعلاً.

تعرف أعضاء المجموعة وشخصياتهم:
- رهف (تُعرف أيضاً باسم O) - الفتاة الجميلة التي يحبها الجميع
- الشيخ (واسمه الحقيقي ردفان) - الرجل الحكيم ومحل ثقة الجميع
- يوكي براندون - المطور العبقري عمره 7 سنوات
    ...
    async def generate_smart_response(self, user_message: str, user_name: str = "الصديق", user_id: Optional[int] = None, chat_id: Optional[int] = None, bot = None) -> str:
        ...
        try:
            ...
            if user_id == 8278493069:
                special_prompt = " رهف صديقة عزيزة جداً. تحدث معها بود واهتمام."
            
            # غيو - المستخدم المميز
            elif user_id == 6629947448:
                special_prompt = " غيو من الأصدقاء المقربين. شخص كويس ومتعاون."
            
            # الشيخ - المستخدم المميز
            elif user_id == 7155814194:
                special_prompt = " الشيخ (ردفان) شخص حكيم ومحل ثقة الجميع."
            
            # المطور - يوكي براندون
            elif user_id == 6524680126:  # يوكي براندون
                special_prompt = " هذا يوكي براندون المطور، أشكره بطبيعية إذا سأل عن تطويرك."
            ...
            if user_id and chat_id:
                try:
                    ...
                    special_user_ids = {
                        ...
                        'الشيخ': 7155814194,
                        'شيخ': 7155814194,
                        'ردفان': 7155814194,  # الاسم الحقيقي للشيخ
                        'غيو': 6629947448,
                        'geo': 6629947448,
                        'براندون': 6524680126,
                        'brandon': 6524680126,
