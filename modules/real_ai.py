...
كن طبيعياً ومرحاً. استخدم الاسم الحقيقي للشخص. رد بالعربية مع إيموجي قليلة."""
    ...
    async def handle_admin_questions(self, user_message: str, user_name: str, user_id: Optional[int], chat_id: Optional[int], bot) -> Optional[str]:
        ...
        # أسئلة عن مالك المجموعة الحقيقي (ليس النظام)
        group_owner_keywords = ['مالك المجموعة', 'مالك ذي المجموعة', 'صاحب المجموعة', 'مالك الجروب', 'صاحب الجروب', 'مالك الكروب', 'صاحب الكروب', 'مالك القروب', 'صاحب القروب', 'مالك قروب', 'صاحب قروب']
        question_keywords = ['مين', 'منو', 'who', 'من هو', 'ما اسم', 'اسم']
        
        # أسئلة عن يوكي كنظام (فقط إذا ذكر يوكي صراحة مع كلمات تقنية ولم يسأل عن المجموعة)
        yuki_system_keywords = ['البوت', 'النظام', 'التطبيق', 'البرنامج', 'الذكي', 'الاصطناعي']
        if ('يوكي' in message_lower and 
            any(admin_word in message_lower for admin_word in ['مدير', 'مالك', 'صاحب', 'مطور', 'مبرمج']) and 
            any(q_word in message_lower for q_word in question_keywords) and
            any(sys_word in message_lower for sys_word in yuki_system_keywords) and
            not any(group_word in message_lower for group_word in ['المجموعة', 'الجروب', 'الكروب', 'القروب', 'قروب', 'كروب', 'جروب'])):
            import random
            responses = [
                f"أهلاً {user_name}! 😊\n\n🤖 أنا يوكي، البوت الذكي المساعد! أنا مطور لمساعدة الأعضاء في المجموعات وتقديم معلومات مفيدة.\n\n🔧 **قدراتي كنظام:**\n• الوصول لقواعد البيانات لعرض المعلومات\n• تنفيذ الأوامر والطلبات\n• إدارة النظام المالي والألعاب\n• مساعدة الأعضاء والإجابة على الأسئلة\n\nأنا مصمم لخدمتكم! 💙",
                f"هلا {user_name}! 👋\n\n🎯 أنا يوكي، مساعدك الذكي! تم تطويري لأكون نظام شامل يساعد المجموعات.\n\n⚙️ **ما أقدر أعمله:**\n• إدارة بيانات الأعضاء\n• عرض الإحصائيات والمعلومات\n• تشغيل الألعاب والأنشطة\n• الرد على الأسئلة بذكاء\n\nهدفي مساعدتكم وجعل تجربتكم أفضل! 🌟"
            ]
            return random.choice(responses)
        
        # توضيح للبوت بدون ذكر يوكي
        if any(word in message_lower for word in ['البوت', 'بوت']) and any(admin_word in message_lower for admin_word in ['مالك', 'صاحب']) and any(q_word in message_lower for q_word in question_keywords):
            return f"أهلاً {user_name}! 😊\n\nتقصد مالك يوكي (البوت) أم مالك المجموعة؟ 🤔\n\nإذا تقصد مالك يوكي، فأنا يوكي ومالك النظام!\nوإذا تقصد مالك المجموعة، اسأل: \"مين مالك المجموعة؟\""
        
        # فحص إذا كان السؤال عن مالك المجموعة وليس البوت
        if (any(group_word in message_lower for group_word in group_owner_keywords) or (
            any(admin_word in message_lower for admin_word in ['مالك', 'صاحب']) and 
            any(q_word in message_lower for q_word in question_keywords) and
            chat_id and  # في مجموعة
            'يوكي' not in message_lower and 'البوت' not in message_lower and 'بوت' not in message_lower and
            not any(sys_word in message_lower for sys_word in yuki_system_keywords)  # ليس سؤال تقني
        )):
            return await self.handle_group_owner_inquiry(user_name, user_id, chat_id, bot)
        
        # أسئلة عن بيانات النظام أو المعلومات - فقط للطلبات الصريحة والمحددة للمعلومات الإدارية
        admin_data_keywords = ['إحصائيات النظام', 'بيانات النظام', 'تقرير النظام', 'معلومات إدارية', 'إحصائيات إدارية', 'تقرير المجموعة', 'إحصائيات المجموعة', 'بيانات المجموعة']
        explicit_admin_request = (
            any(keyword in message_lower for keyword in admin_data_keywords) or
            (any(word in message_lower for word in ['عرض', 'أعرض', 'اعرض', 'شوف']) and 
             any(word in message_lower for word in ['إحصائيات', 'احصائيات', 'بيانات', 'تقرير']) and
             any(word in message_lower for word in ['النظام', 'المجموعة', 'إداري', 'إدارية']))
        )
        
        if explicit_admin_request:
            if chat_id and bot and user_id:
                admin_system_info = await self.get_admin_system_info(user_id, chat_id, bot)
                return f"أهلاً {user_name}! 😊\n\nهذا تقرير شامل عن حالة النظام والمجموعة:\n\n{admin_system_info}"
            else:
                return f"أهلاً {user_name}! 😊\n\nأحتاج معلومات المجموعة لعرض التقرير الإداري المفصل."
    ...
    async def handle_group_owner_inquiry(self, user_name: str, user_id: Optional[int], chat_id: Optional[int], bot) -> str:
        """التعامل مع أسئلة مالك المجموعة الحقيقي"""
        if not chat_id:
            return f"أهلاً {user_name}! 😊\n\nهذا سؤال عن مالك المجموعة، لكن نحن مش في مجموعة حالياً.\n\nإذا كنت تسأل في مجموعة معينة، استخدم هذا الأمر داخل تلك المجموعة!"
        
        try:
            from config.hierarchy import get_group_admins, sync_telegram_admins_to_local
            from database.operations import get_user
            
            # مزامنة بيانات التليجرام أولاً للحصول على أحدث معلومات المالكين
            await sync_telegram_admins_to_local(bot, chat_id)
            
            # جلب معلومات المالكين من النظام
            group_admins = get_group_admins(chat_id)
            owners = group_admins.get('owners', [])
            
            if not owners:
                return f"أهلاً {user_name}! 😊\n\n🤔 **مالك المجموعة:**\n\nما قدرت أحدد مالك هذه المجموعة حالياً. ممكن المالك مو مسجل في النظام بعد، أو في مشكلة مؤقتة في جلب البيانات.\n\nجرب مرة ثانية بعد شوية! 🔄"
            ...
            if len(owners) == 1:
                ...
                try:
                    ...
                    if owner_data:
                        ...
                        return f"أهلاً {user_name}! 😊\n\n👑 **مالك هذه المجموعة:**\n\n🆔 المعرف: {owner_id}\n\nالمالك موجود لكن بياناته مش متاحة في النظام حالياً."
                except Exception as e:
                    return f"أهلاً {user_name}! 😊\n\n👑 **مالك هذه المجموعة:**\n\n🆔 المعرف: {owner_id}\n\nما قدرت أجيب تفاصيل أكثر حالياً، بس هذا هو المالك! 👍"
                ...
                for i, owner_id in enumerate(owners, 1):
                    try:
                        ...
                        if owner_data:
                            ...
                            response += f"{i}. **المستخدم {owner_id}**\n"
                            response += f"   🆔 المعرف: {owner_id}\n\n"
                    except:
                        response += f"{i}. **المستخدم {owner_id}**\n"
                        response += f"   🆔 المعرف: {owner_id}\n\n"
                
                response += "هؤلاء هم أصحاب المجموعة! 🏆"
                return response
    ...
    async def generate_smart_response(self, user_message: str, user_name: str = "الصديق", user_id: Optional[int] = None, chat_id: Optional[int] = None, bot = None) -> str:
        ...
        try:
            ...
            if user_id:
                ...
                history = await conversation_memory_sqlite.get_conversation_history(user_id, limit=15, chat_id=chat_id)
                if history:
            ...
            if ai_response and len(ai_response.strip()) > 0:
                ...
                if user_id:
                    try:
                        ...
                        from modules.conversation_memory_sqlite import conversation_memory_sqlite
                        await conversation_memory_sqlite.save_conversation(user_id, user_message, ai_response, chat_id)
