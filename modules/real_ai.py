...
كن طبيعياً ومرحاً. استخدم الاسم الحقيقي للشخص. رد بالعربية مع إيموجي قليلة."""
    ...
    async def handle_admin_questions(self, user_message: str, user_name: str, user_id: Optional[int], chat_id: Optional[int], bot) -> Optional[str]:
        ...
        # أسئلة عن يوكي كنظام (فقط إذا ذكر يوكي صراحة)
        if 'يوكي' in message_lower and any(admin_word in message_lower for admin_word in ['مدير', 'مالك', 'صاحب']) and any(q_word in message_lower for q_word in question_keywords):
            import random
            responses = [
