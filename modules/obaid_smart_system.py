...
class ObaidSmartSystem:
    ...
    def __init__(self):
        ...
        self.smart_responses = {
            'greetings': [
                "أهلاً! أنا عبيد، عمري 7 سنوات وأحب الجولف! 🏌️‍♂️",
                "مرحبا! أنا عبيد الصغير، أحب ألعاب الجولف كثيراً! ⛳",
                "أهلاً وسهلاً! عبيد في الخدمة، عندي 7 سنين وأحب الرياضة! 🎮"
            ],
            'questions': [
                "إيش تقصد؟ أنا ما فهمت! 🤔",
                "ممكن توضح أكثر؟ أنا صغير! 😅", 
                "أقدر أساعد بس وضح أكثر! 👦"
            ],
            'thanks': [
                "العفو! أنا عبيد دائماً مستعد أساعد! 😊",
                "لا شكر على واجب، أنا عبيد الطيب! 🌟",
                "أهلاً وسهلاً، عبيد في خدمتك! 💪"
            ],
            'compliments': [
                "شكراً! أنا بحب لما الناس تمدحني! 😄",
                "يسلمو! أنا عبيد المتفائل! ✨",
                "أي والله، أنا ولد طيب! 🥰"
            ],
            'golf_talk': [
                "وااااي! انت تحب الجولف مثلي؟! 🏌️‍♂️⛳",
                "الجولف رياضة رائعة! أنا ألعب كل يوم! 🏆",
                "يللا نلعب جولف مع بعض! أنا محترف! 💪⛳"
            ],
            'age_talk': [
                "أي صح، أنا عمري 7 سنين! كبرت شوي! 😎",
                "أنا عبيد الصغير، بس ذكي! 🧠✨", 
                "7 سنين وما زلت أتعلم أشياء جديدة! 📚"
            ],
            'random': [
                "أنا عبيد! وين أصدقائي؟ 👀",
                "أحب أكون مع الجماعة هنا! 🤗",
                "عبيد موجود ومستعد للمرح! 🎉",
                "أي شخص يبي يلعب معي؟ 🎮",
                "الحمدلله على يوم جديد! 🌅"
            ]
        }
    ...
    async def should_obaid_respond_to_mention(self, message: Message) -> Optional[str]:
        ...
        if any(word in text_lower for word in ['أين', 'وين', 'موجود']):
            return f"أنا هنا! عبيد موجود! 👋 أهلاً {replier_name}!"
            
        elif any(word in text_lower for word in ['كيف', 'شلون', 'كيفك']):
            if user_info and 'last_interaction' in user_info:
                return f"أنا بخير {replier_name}! آخر مرة تكلمنا كان عن {user_info.get('topic', 'أشياء حلوة')}! 😊 وانت كيفك؟"
            else:
                return f"أنا بخير {replier_name}! عبيد متفائل ونشيط! 😊 وانت كيفك؟"
            
        elif any(word in text_lower for word in ['جولف', 'golf', 'لعب', 'رياضة']):
            return f"واااي {replier_name}! تحب الجولف مثلي؟ يللا نلعب! 🏌️‍♂️⛳"
            
        elif any(word in text_lower for word in ['عمر', 'سنة', 'كبير', 'صغير']):
            return f"أنا عمري 7 سنين {replier_name}! بس ذكي وأحب أتعلم! 🧠✨"
            
        elif any(word in text_lower for word in ['من', 'مين', 'تعرف']):
            if user_info:
                interests = user_info.get('interests', [])
                if interests:
                    return f"أنا أعرف {replier_name}! انت تحب {', '.join(interests[:2])}! أنا عبيد صديقك! 🤗"
                else:
                    return f"أنا عبيد! انت {replier_name} صديقي! تعال نتعرف أكثر! 👦✨"
            else:
                return f"أنا عبيد! انت {replier_name}، صح؟ يللا نصير أصدقاء! 🤝"
            
        else:
            # رد عام مع استخدام معلومات المستخدم إن وُجدت
            if user_info and random.random() < 0.4:  # 40% احتمال لاستخدام المعلومات
                return f"أهلاً {replier_name}! أنا عبيد وأتذكرك! نتكلم مرة ثانية؟ 😊"
            else:
                general_responses = [
                    f"أيوه {replier_name}؟ أنا عبيد! إيش تبي؟ 😊",
                    f"نعم {replier_name}! عبيد في الخدمة! 🌟",
                    f"أهلاً {replier_name}! قل لعبيد إيش تريد! 👦",
                    f"مرحبا {replier_name}! أنا عبيد، كيف أساعدك؟ 🤝"
                ]
